	...
	$.ajax({
		...
		success : function(res) {
			if(res.code==200){
				...
				var userimg = user.image.indexOf('http')!=-1?user.image:(urlpath+user.image);
				var html = `<div id="uploadList_1" class="upload_append_list">
					...
					<a style="height:85px;width:100px;" href="#" class="imgBox">
						<div class="uploadImg" style="width:85px;max-width:85px;max-height:75px;">
							<img id="uploadImage_1" class="upload_image" src="${userimg}" style="width:expression(this.width > 85 ? 85px : this.width);">
						</div>
					</a>
	...
	if(id != '') {// 编辑
		if(img=='null' || img==null){
			...
			$.post(urlpath+'User/updateuser',{
				...
				tool2(res.code==200,"myModal","用户信息修改成功",res.message);
		...
		if(img=='null' || img==null){
			...
			var paramJson = {
			 ...
			 $.post(urlpath+"User/add",{
				 ...
				 tool2(res.code==200,"myModal","用户信息添加成功",res.message);
				 table.ajax.reload(null, false);
				 img="";
			 });	
		}
