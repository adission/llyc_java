...
//            $('td', row).eq(2).html("<a href='gatePloe.html?id="+data.id+"' class='text-primary'>"+data.gate_name+"</a>");
            $('td', row).eq(-1).html(' <div class="btn-group"> ' +
            		...
            		'<button type="button" class="btn btn-default openCard" onclick="location.href=\'gatePloe.html?id='+data.id+'\'" title="权限管理"><i class="fa fa-floppy-o text-primary"></i></button> ' +
                    '<button type="button" class="btn btn-default start" title="复位"><i class="fa fa-power-off text-primary"></i></button> ' +
                    '<button type="button" class="btn btn-default alOpen" title="常开"><i class="fa fa-check-circle-o text-info"></i></button> ' +
                    '<button type="button" class="btn btn-default alClose" title="常闭"><i class="fa fa-ban text-danger"></i></button>' +
                    '<button type="button" class="btn btn-default resetOpen" title="重启"><i class="fa fa-repeat text-success"></i></button>' +
    ...
    $('#searchBtn').on('click',function(res){
		paramJson = {
			"gate_name": Trim($('#gatename').val()),
			"gate_no": Trim($('#gateno').val()),
			"token": getCookie('token')
		};
    ...
    $("#checkAll").on("click", function () {
        if ($(this).prop("checked") === true) {
            ...
            $('#gateList tbody tr').addClass('selected');
        } else {
            $("input[name='checkList']").prop("checked", false);
            $('#gateList tbody tr').removeClass('selected');
        }
    });
  ...
  //点击展开详情
    function format ( d ) {
        return `<form class="form-inline col-xs-10 formDetail">
    ...
    // 删除一个
    $('#gateList tbody').on('click', '.delRow', function () {
...
function delGate(id){
	$.post(urlpath+'GateList/delete',{
		...
		tool(res.code==200,"闸机删除成功",res.message);
	});
}
...
function resetGate(id){
	$.post(urlpath+'GateList/restart',{
		...
		tool(res.code==200,"重启闸机成功",res.message);
	});
}
//启动闸机接口
function startGate(id){
	$.post(urlpath+'GateList/allnormal',{
		...
		tool(res.code==200,"闸机启动成功",res.message);
	});
}
// 常开接口
function alwaysOpen(id){
	$.post(urlpath+'GateList/allopen',{
		...
		tool(res.code==200,"设置常开闸机成功",res.message);
	});
}
...
function alwaysClose(id){
	$.post(urlpath+'GateList/allclose',{
		...
		tool(res.code==200,"设置常闭闸机成功",res.message);
	});	
}
