	...
	if(id != '') {// 编辑
		var paramJson = {
				"id": id,
				 ...
				 "gate_name": Trim($('input[name="gatename"]').val()),
				 "cross_flag": $('select[name="cross_flag"] option:selected').val(),
				 "sn": Trim($('input[name="sn"]').val()),
				 "connect_id": $('select[name="connect_id"] option:selected').val(),
				 "gate_no": Trim($('input[name="gateno"]').val()),
				 "location": Trim($('input[name="location"]').val()),
				 ...
				 "common": Trim($('input[name="common"]').val()),
				 "connect_port": Trim($('input[name="connect_port"]').val()),
				 "connect_ip": ip ,
		...
		$.post(urlpath+"GateList/update",{
			...
			tool2(res.code==200,"myModal","闸机信息编辑成功",res.message);
		});
	}else {//添加
		var paramJson = {
		 ...
		 $.post(urlpath+"GateList/add",{
			 ...
			 tool2(res.code==200,"myModal","闸机添加成功",res.message);
		 });
	}
