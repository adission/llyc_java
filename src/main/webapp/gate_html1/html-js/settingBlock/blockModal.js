...
function surBlock(){
	...
	if(id != '') {// 编辑
		var paramJson = {
				"welcome": Trim($('input[name="welcome"]').val()),
				"group_name": Trim($('input[name="groupname"]').val()),
				 "id": id
		 };
		$.post(urlpath+'Gategroup/update',{
			...
			tool2(res.code==200,"myModal","闸机分区信息修改成功",res.message);
		});
	}else {// 新增用户
		var paramJson = {
				"welcome": Trim($('input[name="welcome"]').val()),
		 ...
		 $.post(urlpath+"Gategroup/add",{
			 ...
			 tool2(res.code==200,"myModal","闸机分区信息添加成功",res.message);
		 });	
	}
