...
/**
 * Created by wangyanling on 2017/8/29.
 */
var table;
...
$(function(){
   ...
   table = $('#table-detail').dataTable($.extend(true,{},CONSTANT.DATA_TABLES.DEFAULT_OPTION,{
       ...
       "createdRow": function ( row, data, index ) {
            ...
            //给当前行加样式$(row) $('td', row).eq(2) 
        		$('td', row).eq(6).html(' <div class="btn-group"> ' +
                        ...
                        '<button type="button" class="btn btn-default delRow" title="删除" data-id=\''+data.id+'\'><i class="glyphicon glyphicon-remove text-danger"></i></button>' +
                        ' </div>	');
        },
        // 表格重绘完成后
...
function openModal(id) {
	...
	$('#myModal').load('html-js/userManage/userModal.html',function(result) {
		if(id == '') {
...
function delUser(id){
	$.post(urlpath+'User/deleteuser',{
		...
		tool(res.code==200,"用户删除成功","用户删除成功");
	});
}
