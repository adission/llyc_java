...
function setMapId(){
	$.post('../posfloor/getOneFloorInfo',{
		...
		if(res.code=="200"){
			setCookie("mapId",res.data.id);
			setCookie("map",JSON.stringify(res.data))
		}
	});
}
                ...
                for(var i=0;i<mapList.length;i++){
                	if(i==0){
                		var html = `<li class="activeMap">
	                        ...
	                        <div class="directory-info-row" onclick="changeMap(this,'${ mapList[i].id }','${ mapList[i].layer_name }','${mapList[i].bjtx}')" data-obj="${ mapList[i].min_x }-${ mapList[i].min_y }-${ mapList[i].max_x }-${ mapList[i].max_y }">
	                            <div class="media">
	                                <a class="pull-left" >
...
function changeMap(tag,id,name,url){
	...
	var obj = {"id": id,"bjtx": url,"name": name, "min_x": ex[0],"min_y": ex[1],"max_x": ex[2],"max_y": ex[3]};
	setCookie("mapId",id);
	setCookie("map",JSON.stringify(obj));
	// 新图层的基站，区域，摄像头
	getAreaList(id);
	baseList(id);
	videoList(id);
