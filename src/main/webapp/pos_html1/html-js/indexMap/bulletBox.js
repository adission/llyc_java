...
//	  window.setTimeout(function() {
	      ...
	      map.forEachFeatureAtPixel(evt.pixel, function(feature) {
	    	  if(feature.getGeometryName()=="user"){
		          ...
		          //alert(feature.get("color"));
		          //alert(feature.get("userId")+"   "+feature.get("typeName"));
		          console.log(feature.get("userId")+"   "+feature.get("typeName"))
		          var types=feature.get("typeName");
			      ...
			      if(types=="工作人员"){
			          ...
			          $.ajax({
		                  ...
		                  $("#cid").text(cid)  
				   		  ...
				   		  $("#native_place").text(datas.hometown);//这个需要表中添加字段
				          ...
				          success:function(result){
				        	 //alert(JSON.stringify(result))
				      	    ...
				      	    $("#user_worker").css("min-width","428px");	 
				   			...
				   			$("#birthday1").text("离开时间:");
				   			//getCarType(datas.car_type);
				   			$("#nation").text(datas.cartype); 
				   			$("#nation1").text("车辆类型");
				   			...
				   			$("#mobile").text(datas.mobile); 
				   			$("#mobile1").text("联系方式:");
				   			//getPapType(datas.pap_type);
				   			//alert(pap_types)
				   			$("#department").text(datas.paptype);
...
/*//此方法后面会去掉
function getPapType(pap_type){
	$.post("/lanlyc/PapTypes/selectAllPapTypes",{
		"token": getCookie('token')
	},function(res){
		//alert(JSON.stringify(res));
		if(res.code==200){
			for(var i=0;i<res.data.length;i++){
				if(res.data[i].value==pap_type){
					$("#department").text(res.data[i].name);
		   			$("#department1").css("margin-left","-29px");
		   			$("#department1").text("证件类型:");
...
//此方法后面会去掉
function getCarType(car_type){
	//获取车辆类型列表
	$.post("/lanlyc/CarTypes/selectAllCarTypes",{
		"token": getCookie('token')
	},function(res){
		//alert(JSON.stringify(res));
		if(res.code==200){
			for(var i=0;i<res.data.length;i++){
				if(res.data[i].value==car_type){
					$("#nation").text(res.data[i].name); 
		   			$("#nation1").text("车辆类型");
